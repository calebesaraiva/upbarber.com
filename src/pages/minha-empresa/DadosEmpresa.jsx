...
export default function DadosEmpresa() {
  ...
  const [form, setForm] = useState({
    name: '', cnpj: '', phone: '', whatsapp: '', email: '',
  ...
  const save = async () => {
    setSaving(true);
    try {
      await settingsService.updateBarbershop(form);
  ...
  return (
    <div>
      ...
      <div className="max-w-2xl space-y-4">
        <div className="card space-y-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <button className="btn-secondary text-xs py-1.5 mt-2" onClick={() => addToast('Em breve', 'info')}><Upload size={11} /> Enviar logo</button>
            </div>
          </div>
        </div>
        ...
        <button className="btn-primary w-full justify-center py-3" onClick={save} disabled={saving}>
